...
def main():
    ...
    # Read 5 columns from transcripts CSV file
    transcripts_df = pd.read_csv(args.baysor,
                                  usecols=["gene",
                                           "cell",
                                           "assignment_confidence"])

    # Replace NaN values in the "cell" column with a default value, such as "unknown"
    transcripts_df["cell"] = transcripts_df["cell"].fillna("unknown")

    # Find distinct set of cells.
    cells = transcripts_df["cell"].astype(str).unique()

    # Now you can sort the cells
    cells.sort()
